...
export class KeycloakAdminClient {
  ...
  get<T>(path: string): Promise<AdminResponse<T>> {
    return this.request<T>("GET", path);
  }

  post<T>(path: string, body?: unknown): Promise<AdminResponse<T>> {
    return this.request<T>("POST", path, body);
  }

  put<T>(path: string, body?: unknown): Promise<AdminResponse<T>> {
  ...
  async request<T>(
    ...
    if (first.status !== 401 || !RETRYABLE_METHODS.includes(method)) {
